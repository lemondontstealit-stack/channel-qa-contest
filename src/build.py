...
print(f"Image compression: {total_size/1024/1024:.1f}MB ({len(encoded)}/{len(all_imgs)} ok)")

# ===== 5. 生成 hidden img 标签 =====
hidden_tags = []
for fn in sorted(encoded.keys()):
    img_id = re.sub(r'\.(png|jpeg|jpg|PNG|JPG|JPEG)$', '', fn)
    b64 = encoded[fn]
    hidden_tags.append(
        f'<img id="i-{img_id}" src="data:image/jpeg;base64,{b64}" style="display:none">'
    )
